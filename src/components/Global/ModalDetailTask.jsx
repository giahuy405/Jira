...
const ModalDetailTask = (props) => {
    ...
    const onSubmit = async (values, actions) => {
        ...
        console.log(values, 'modal create ták2222')
        let newValues = { ...values, projectId: id }
        const res = await dispatch(createTaskAction(newValues));
        // console.log(id, 'iddd')

        res && await dispatch(getProjectDetail(id))
        actions.resetForm();
        props.setOpenModalDetailTask(false)
    }
    ...
    return (
        <div >
            {allProjectKeyword &&
                <Modal
                    className='ModalDetailTask'
                    centered
                    open={props.openModalDetailTask}
                    onOk={() => props.setOpenModalDetailTask(true)}
                    onCancel={() => props.setOpenModalDetailTask(false)}
                    width={620}
                    footer={[]}
                >
                            ...
                            <Form style={{ lineHeight: '16px' }} >
                                <div className='grid grid-cols-2 gap-4 mt-4'>
                                    <div className='col-span-2 flex gap-3 '>
                                        <div className='w-1/2'>
                                            <label htmlFor="" className='cursor-pointer text-sm text-gray-400'>Project name</label>
                                            {/* <Select
                                                className='uppercase'
                                ...
                                <div className="mt-6">
                                    <div className='flex justify-end'>
                                        <button
                                            ...
                                            key="cancel"
                                            onClick={() => props.setOpenModalDetailTask(false)}>
                                            Cancel
                                        </button>
                                        {isSubmitting ?
                                            <Button
